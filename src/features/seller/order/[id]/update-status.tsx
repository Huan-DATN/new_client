...
import { AlertCircle, Check, CheckCircle, Clock, Loader2, Package, ShoppingBag, Truck } from 'lucide-react';
...
function UpdateStatus({ id, currentStatus, allStatuses }: UpdateStatusProps) {
  ...
  const currentStep = statusSequence.indexOf(currentStatus.type as OrderStatusEnum);
      ...
      <CardContent className="space-y-6">
          ...
          <div className="space-y-6">
            {progressSteps.map((step, index) => (
              <div key={index} className="flex items-start gap-3">
                <div className={`flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center ${step.isActive ? 'bg-primary text-white' : 'bg-gray-200'} transition-colors duration-300`}>
                  {getStatusIcon(step.type)}
                </div>
                <div className="flex flex-col gap-1 mt-1">
                  <OrderStatusBadge status={step} size="sm" />
                  <p className="text-sm text-muted-foreground">
                    {step.type === OrderStatusEnum.PENDING && 'Đơn hàng đang chờ xác nhận'}
                    {step.type === OrderStatusEnum.CONFIRMED && 'Đơn hàng đã được xác nhận'}
                    {step.type === OrderStatusEnum.PROCESSING && 'Đang chuẩn bị đơn hàng'}
                    {step.type === OrderStatusEnum.SHIPPED && 'Đơn hàng đang được giao'}
                    {step.type === OrderStatusEnum.DELIVERED && 'Đơn hàng đã giao thành công'}
                  </p>
                </div>
              </div>
            ))}
        ...
        <div className="rounded-md border p-4 bg-muted/20">
          <div className="flex items-center gap-2 text-sm">
            ...
            <p className="font-medium">Trạng thái hiện tại: <span className="font-semibold text-primary">{currentStatus.name}</span></p>
          </div>
        </div>

        {/* Action Buttons */}
        <div className="pt-4 space-y-3">
          {nextStatus && !canCompleteDirectly && (
            <Button
            ...
            >
              {loading ? (
                ...
                <>
                  {getStatusIcon(nextStatus.type)}
                  <span className="ml-2">Cập nhật thành "{nextStatus.name}"</span>
                </>
              )}
            </Button>
          )}
