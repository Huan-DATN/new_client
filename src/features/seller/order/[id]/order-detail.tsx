...
import { AlertTriangle, ArrowLeft, Calendar, CreditCard, MapPin, Receipt, ShoppingBag, User } from 'lucide-react';
...
async function OrderDetail({ id }: { id: number }) {
	...
	const canCancel = [OrderStatusEnum.PENDING, OrderStatusEnum.CONFIRMED].includes(latestStatus.type as OrderStatusEnum);
	const canConfirm = latestStatus.type === OrderStatusEnum.PENDING;
		...
		<div className="container max-w-5xl mx-auto py-6 space-y-6">
			...
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
						...
						<CardContent>
							<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
								...
								<div className="space-y-3">
									<h3 className="text-sm font-medium text-muted-foreground">Thông tin đơn hàng</h3>
									...
									<div className="flex items-start gap-2 text-sm">
										...
										<div>
											...
											<p className="text-muted-foreground">{getDateFormat(new Date(order.createdAt))}</p>
										</div>
									</div>
									...
									<div className="flex items-start gap-2 text-sm">
										...
										<div>
											...
											<p className="text-muted-foreground">COD (Thanh toán khi nhận hàng)</p>
										</div>
									</div>
								</div>
								...
								<div className="space-y-3">
									<h3 className="text-sm font-medium text-muted-foreground">Thông tin người nhận</h3>
						...
						<CardContent>
							<div className="space-y-4">
								{order.items.map((item) => (
									<div key={item.id} className="flex items-start gap-4 py-4 first:pt-0 last:pb-0 border-b last:border-0">
										<div className="relative h-20 w-20 rounded-md overflow-hidden border bg-muted flex-shrink-0">
											<Image
												src={item.product?.images[0]?.publicUrl ?? '/placeholder.png'}
										...
										<div className="font-medium">{getPriceFormat(item.unitPrice * item.quantity)}</div>
									</div>
								))}
								...
								<div className="flex justify-between items-center pt-2">
									<p className="font-medium">Tổng cộng ({order.items.length} sản phẩm):</p>
									<p className="font-bold text-primary text-lg">{getPriceFormat(order.total)}</p>
								</div>
							</div>
						</CardContent>
					</Card>
				...
				<div className="space-y-6">
					{/* Action card */}
					{latestStatus.type !== OrderStatusEnum.CANCELLED && latestStatus.type !== OrderStatusEnum.DELIVERED && (
						<>
							...
							{canCancel && (
								<Card>
									<CardHeader className="pb-3">
										<CardTitle className="text-lg text-destructive">Tùy chọn thêm</CardTitle>
									</CardHeader>
									<CardContent>
										<TooltipProvider>
											<Tooltip>
					...
					{latestStatus.type === OrderStatusEnum.PENDING && (
						<Card>
							<CardHeader className="pb-3">
					...
					<Card>
						...
						<CardContent>
							<OrderTimeLine id={order.id} data={order.OrderStatus} isSeller={true} />
						</CardContent>
					</Card>
				</div>
			</div>
