...
export default function ChartDashboardCard({
  ...
  const [chartData, setChartData] = useState<StatisticResponseType['data'] | []>([]);

  useEffect(() => {
    statisticRequest.getStatistic(sessionToken, {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Biểu đồ về đơn hàng và doanh thu tháng gần đây</CardDescription>
      </CardHeader>
      <CardContent>
        <Tabs defaultValue="sales">
          <TabsList className="mb-4 justify-center">
            <TabsTrigger value="sales" className='border-2 border-gray-300 rounded-md'>Area Chart</TabsTrigger>
            <TabsTrigger value="orders" className='border-2 border-gray-300 rounded-md'>Bar Chart</TabsTrigger>
          </TabsList>
